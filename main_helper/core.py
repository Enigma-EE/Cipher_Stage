...
class LLMSessionManager:
    ...
    async def send_lanlan_response(self, text: str, is_first_chunk: bool = False):
        ...
        try:
            ...
            logger.error(f"💥 WS Send EE Response Error: {e}")
    ...
    async def system_timer(self):  # 定期向EE发送心跳，允许EE主动向用户搭话。
            ...
            if input_type == 'audio':
                ...
                try:
                    if isinstance(data, str) and data.strip():
                        # 记录并广播用户文本为“转录”，用于前端与监控展示
                        text = data.strip()
                        await self.handle_input_transcript(text)
                        # 打断当前回复，避免模型继续说话时插入冲突
                        try:
                            if hasattr(self.session, 'handle_interruption'):
                                await self.session.handle_interruption()
                        except Exception:
                            pass
                        # 显式触发模型基于用户文本进行自然回复，避免逐字复述
                        instructions = (
                            f"请继续与用户对话。用户说：{text}。"
                            f"请自然回答，不要逐字复述用户原句，保持本角色（{self.lanlan_name}）的口吻。"
                        )
                        await self.session.create_response(instructions, skipped=False)
                    else:
